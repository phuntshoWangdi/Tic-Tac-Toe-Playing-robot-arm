...
locations = [
	'g1 x157 y126 z-20', 'g1 x135 y145 z-20', 'g1 x116 y167 z-20',
...
def stateOfBoard():
	...
	for i in range(len(img_lst)):
		...
		result_o = cv2.matchTemplate(cv2.resize(img_lst[i],(120,120)), template_o, cv2.TM_CCOEFF_NORMED)
		result_x = cv2.matchTemplate(cv2.resize(img_lst[i],(120,120)), template_x, cv2.TM_CCOEFF_NORMED)
...
def whosTurn():
	...
	result_o = cv2.matchTemplate(turn_img, turn_o, cv2.TM_CCOEFF_NORMED)
	result_x = cv2.matchTemplate(turn_img, turn_x, cv2.TM_CCOEFF_NORMED) 
